...
public class CriAtomExOutputAnalyzer : CriDisposable
{
	...
	public struct Config
	{
		 ...
		 * Enables the acquisition of output data.
		 * When using the function, specify True in this flag and
		 * set numCapturedPcmSamples to a positive number.
	 ...
	 * When using this function, create a module setting enableLevelmeter to True in Config.
	 	...
	 	需要在创建 analyzer 之前, 先在 Config 中设置 enableLevelmeter 为 true;
	 ...
	 * you need to convert the value returned by this function into a decibel value.(分贝值)
	 ...
	 * and a numCapturedPcmSamples to a positive number in Config.
	 ---
	 需要在创建 analyzer 之前, 先在 Config 中设置 enablePcmCapture 为 true,
	 同时配置  numCapturedPcmSamples;
	 ...
	 * Currently, the only channels that can be acquired are L/R. For ch, specify 0 or 1.
